...
- サーバー接続エラー「The operation is not allowed on non-connected sockets」の修正
  - Python側のサーバーがTCPモードで起動していなかった問題を特定
  - `server.py`を修正してTCPトランスポート（ポート6500）で起動するよう変更
  - エラー発生時のフォールバックとしてstdioモードを追加
  - デバッグログを追加して接続状態の把握を容易に
...
4. **サーバーの起動**:
   ...
   python server.py
   ```
   "Starting MCP TCP server on localhost:6500" というメッセージが表示されることを確認します。
